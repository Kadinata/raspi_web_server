...
describe('Auth Class', () => {
  ...
  describe('object initialization', () => {
    it('should initializes an instance correctly', () => {
      ...
      const auth = new Auth(User, TEST_JWT_SECRET);
  ...
  describe('user registration', () => {
    it('should allow multiple new users to register', async () => {
      const mock_create_user = jest.spyOn(User, 'create');
  ...
  describe('password update', () => {
    ...
    it('should handle password update failure gracefully', async () => {
      ...
      const mock_user_update_password = jest.spyOn(User, 'updatePassword')
  ...
  describe('finding a user by ID', () => {
    it('should find the user if it exists', async () => {
      ...
      const result = await auth.findUserById(user.id);
      expect(result).toEqual(user);
    });
    ...
    it('should return null if the user does not exist', async () => {
      ...
      const result = await auth.findUserById(0xBADDF00D);
      expect(result).toEqual(null);
    });
  });
});
